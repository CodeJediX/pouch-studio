...
renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;host.appendChild(renderer.domElement);
...
scene.add(new THREE.HemisphereLight(0xffffff,0x586273,.8));
function light(c,i,p){const l=new THREE.DirectionalLight(c,i);l.position.set(...p);scene.add(l)}
light(0xffffff,1.6,[1,2,3]);light(0xcbdfff,.7,[-2,0,2]);light(0xffffff,1.5,[1,1,-2]);
...
new GLTFLoader().load('./Laptop_Pouch.glb?v=4',g=>{scene.add(g.scene);g.scene.traverse(o=>{if(o.isMesh){o.material.envMapIntensity=.25;}});loading.hidden=true;},undefined,()=>{loading.textContent='The model could not load. Please refresh the page.';});
